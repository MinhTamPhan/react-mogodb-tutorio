...
function connect() {
  mongoose.connect("mongodb://localhost/module_4_assignment_01", {
    useMongoClient: true
  });
}
...
app.post("/accounts", function(req, res) {
  ...
  userAccount.save((err, result) => {
    if (err) {
      ...
      console.log("post accounts saved");      
      console.info(results);      
      res.send({ saved: true });
    }
  });
    ...
    if (err) {
      ...
      if (result) {
        ...
        result.save((innerErr, innerResult) => {
          ...
          console.info(results);
          console.log("put accounts saved");       
          res.send({ saved: true });
        });
      } else {
  ...
  Account.findById(req.params.id, (err, result) => {
    if (err) {
      ...
      if (result) {
        result.remove();
        console.info(results);  
        console.log("delete accounts delete");             
        res.sendStatus(200);
      } else {
        next(new Error("Account not found :-("));
      }
